...
# Папка проекта на некоторых хостингах (Render и т.п.) может быть доступна
# только для чтения, поэтому SQLite-базу храним в системной временной папке —
# она гарантированно доступна для записи в любом окружении.
DB_DIR = os.environ.get('DB_DIR', '/tmp/webcanteen_instance')
os.makedirs(DB_DIR, exist_ok=True)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(DB_DIR, 'cantean.db')
...
def create_initial_data():
    ...
    if not admin:
        ...
        print("Администратор 'admin' создан. Пароль задаётся через переменную окружения ADMIN_PASSWORD (по умолчанию '1234' — смените перед реальным использованием!)")
...
# --- Инициализация БД (выполняется и при локальном запуске, и под gunicorn) ---
